...
{
	unsigned long util, min, max_cap;
	struct rq *rq = cpu_rq(cpu);

	/* SCX (sched_ext) is unavailable in this kernel; use CFS util directly */
	...
	if (boost > util)
		util = boost;
	*out_bw_min = min;
	*out_util = sugov_effective_cpu_perf(cpu, util, min, max_cap);
}
...
{
	/* SCX (sched_ext) is unavailable in this kernel */
	return false;
}
